...
export function CourseCard({ course, variant = 'grid', className = '' }: CourseCardProps) {
  if (variant === 'strip') {
    return (
      <Link
        ...
        className={`card-shadow flex items-center gap-3 rounded-box bg-base-100 p-3 transition-shadow hover:shadow-lg ${className}`}
      >
        <img
  ...
  return (
    <article className={`card-shadow flex flex-col overflow-hidden rounded-box bg-base-100 ${className}`}>
      <img
        src={course.thumbnail}
        alt={course.title}
        loading="lazy"
