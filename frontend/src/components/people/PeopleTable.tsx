...
export function PeopleTable({ rows, onMessage, sendingId }: Props) {
  if (rows.length === 0)
    return <p className="py-8 text-center text-sm text-muted">No results.</p>;
  return (
        ...
        <tbody>
          {rows.map((p) => (
            <tr key={p.userId} className="hover">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="flex items-center gap-1 truncate text-xs text-muted">
                      <FiMail aria-hidden size={11} />
                      {p.email}
                    </p>
                    {p.extra ? (
                      <p className="text-[11px] text-muted">{p.extra}</p>
                    ) : null}
                  </div>
                </div>
              </td>
              ...
              <td>
                {p.status ? (
                  <StatusBadge status={humanize(p.status)} />
                ) : (
              ...
              <td className="text-right">
                <button
                  ...
                  className="btn btn-xs btn-ghost gap-1 text-brand"
                >
                  <FiMessageCircle aria-hidden />
                  {sendingId === p.userId ? "Loading..." : "Message"}
                </button>
              </td>
            </tr>
          ))}
