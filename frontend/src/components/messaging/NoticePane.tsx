...
export function NoticeList({ notices, loading, error, onRetry, selectedId, onPick, search, onSearch, onMarkAll, markingAll, noticeError }: Props) {
 ...
 <input value={search} onChange={(e) => onSearch(e.currentTarget.value)} placeholder="Search notices…" className="input input-sm w-full rounded-full border-line bg-base-100 pl-9 pr-8 text-xs" />
...
export function NoticeDetail({ notice }: { notice: { title: string; body: string | null; type: string; createdAt: string; readAt: string | null } | null }) {
 if (!notice) return <div className="flex h-full min-h-[50vh] items-center justify-center"><EmptyBlock title="Select a notice" hint="Choose a notice on the left to read it here — like Canvas announcements." /></div>;
 return (
