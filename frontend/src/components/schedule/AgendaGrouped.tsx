...
type Props = { sessions: Session[]; onOpen: (id: string) => void; onEdit: (id: string) => void; onCancel: (id: string) => void };

export function AgendaGrouped({ sessions, onOpen, onEdit, onCancel }: Props) {
 ...
 <p className="mx-auto mt-1 max-w-sm text-xs leading-snug text-muted">Try another day, change the class filter, or create a new live class.</p>
 </div>
 );
 }
 return (
 ...
 {g.items.map((s) => (
 <SessionRow key={s.id} session={s} onOpen={() => onOpen(s.id)} onEdit={() => onEdit(s.id)} onCancel={() => onCancel(s.id)} />
 ))}
 </div>
 </div>
 ))}
