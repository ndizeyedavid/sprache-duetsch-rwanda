...
export function ComposerCard({ displayName, avatarUrl, onPosted }: Props) {
 ...
 <input value={title} onChange={(e) => setTitle(e.currentTarget.value)} placeholder="Headline — e.g. Mid-term schedule update" className="input w-full rounded-box border-line bg-base-200 text-sm" />
 <textarea value={body} onChange={(e) => setBody(e.currentTarget.value)} placeholder="Add details (optional)… Shift+Enter for new line" rows={2} className="textarea mt-2 w-full rounded-box border-line bg-base-200 text-sm" />
 <div className="mt-3 flex flex-wrap items-center gap-2">
 <select value={type} onChange={(e) => setType(e.currentTarget.value)} className="select select-sm rounded-full border-line bg-base-100 text-xs">
