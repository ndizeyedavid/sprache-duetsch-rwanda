...
export function ActivityGradingPanel({ lessonId }: Props) {
  ...
  const filtered = (
    (submissions.data ?? []) as unknown as {
      ...
      cheatCount: number;
      cheatLog: { type: string; at: string }[] | null;
    }[]
  ).filter((s) => {
    if (!q.trim()) return true;
    const needle = q.trim().toLowerCase();
    return `${s.activity.title} ${s.student.studentCode} ${s.student.user.firstName} ${s.student.user.lastName} ${s.status}`
      .toLowerCase()
      .includes(needle);
  });
  ...
  async function handleGrade(id: string) {
    ...
    setSaving(true);
    try {
      await gradeActivitySubmission(id, {
        score: Number(score),
        isCorrect,
      ...
      setError(apiErrorMessage(err, "Could not grade."));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="overflow-hidden rounded-box border border-line bg-base-100">
      <div className="border-b border-line bg-base-200/50 px-4 py-3">
        ...
        <p className="mt-1 text-xs text-muted">Tap any submission to grade it — score, correct/incorrect and feedback.</p>
      </div>
      <div className="p-4">
        <div className="relative">
          ...
          <input
            ...
            className="input w-full rounded-full border-line bg-base-100 pl-9 pr-8 text-sm"
          />
          {q ? (
            <button type="button" onClick={() => setQ("")} className="btn btn-ghost btn-xs btn-circle absolute right-1 top-1/2 -translate-y-1/2">
              <FiX aria-hidden />
            </button>
          ) : null}
        </div>
        ...
        {submissions.loading ? (
          ...
          <div className="mt-3">
            <ErrorBlock message={submissions.error} onRetry={submissions.refetch} />
          </div>
        ) : filtered.length === 0 ? (
          <div className="mt-3">
            <EmptyBlock title="No submissions yet" hint="Students' answers appear here once they submit an activity." />
          </div>
        ) : (
          <ul className="mt-3 space-y-2">
            {filtered.map((s) => {
              const active = gradingId === s.id;
              return (
                <li
                  key={s.id}
                  onClick={() => selectForGrading(s)}
                  onKeyDown={(e) => {
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="flex flex-wrap items-center gap-1.5 text-sm font-semibold leading-tight">
                        {s.activity.title}
                        <span className="rounded-full bg-base-200 px-2 py-0.5 text-[11px] font-normal">{s.activity.type}</span>
                        <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${s.status === "GRADED" ? "bg-brand text-white" : "bg-sun text-white"}`}>
                          {s.status}
                        </span>
                      </p>
                      <p className="mt-1 text-xs text-muted">
                        {s.student.user.firstName} {s.student.user.lastName} · {s.student.studentCode} · Attempt #{s.attemptNumber} · {new Date(s.submittedAt).toLocaleString()}
                      </p>
                      <p className="mt-1.5 line-clamp-2 rounded bg-base-200 px-2 py-1 text-xs">
                    ...
                    <span className={`flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium transition ${active ? "bg-brand text-white" : "bg-base-200 text-muted group-hover:bg-brand group-hover:text-white"}`}>
                      <FiEdit2 aria-hidden size={12} />
                      {active ? "Grading" : "Grade"}
                      {active ? <FiChevronUp aria-hidden size={14} /> : <FiChevronDown aria-hidden size={14} />}
                    </span>
                  </div>
                    ...
                    <div onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()} className="mt-3 rounded-box border border-line bg-base-100 p-3 shadow-sm">
                      <div className="grid gap-2 sm:grid-cols-3">
                        <label className="block">
                          ...
                          <input type="number" min={0} max={1} step={1} value={score} onChange={(e) => setScore(e.currentTarget.value)} className="input w-full rounded-full border-line bg-base-100 text-sm" />
                        </label>
                        <label className="flex items-center gap-2 pt-6 text-xs font-medium">
                          <input type="checkbox" className="checkbox checkbox-sm" checked={isCorrect} onChange={(e) => setIsCorrect(e.currentTarget.checked)} />
                          Correct
                        </label>
                      </div>
                      <label className="mt-2 block">
                        <span className="mb-1 block text-xs font-medium">Feedback</span>
