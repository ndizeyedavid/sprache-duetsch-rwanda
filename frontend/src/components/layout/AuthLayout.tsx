    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="relative hidden flex-col justify-between overflow-hidden bg-night p-10 text-white lg:flex">
        ...
        <div className="relative max-w-sm">
          ...
          <h2 className="mt-4 text-3xl font-semibold leading-snug">{copy.heading}</h2>
          <p className="mt-3 text-sm leading-relaxed text-white/70">{copy.body}</p>
          <dl className="mt-8 grid grid-cols-3 gap-4">
            {STATS[portal].map((stat) => (
              <div key={stat.label}>
                <dt className="text-[11px] uppercase tracking-wide text-white/60">{stat.label}</dt>
                <dd className="text-lg font-semibold">{stat.value}</dd>
              </div>
            ))}
          </dl>
        </div>
        <p className="relative text-xs text-white/50">© 2025 Deutsch Sprache RW</p>
      </div>
