      ...
      <div className="w-full max-w-md">
        ...
        <Panel>
          ...
          {result.loading ? (
            ...
            <div>
              <ErrorBlock message={result.error ?? 'Verification failed.'} onRetry={result.refetch} />
              <p className="mt-2 font-mono text-xs text-muted">{code}</p>
            </div>
          ) : (
            <div className="mt-2">
              <StatusBadge status={result.data.valid ? 'Valid' : humanize(result.data.status)} />
              <h1 className="mt-3 text-xl font-semibold">{result.data.studentName}</h1>
              ...
              <dl className="mt-4 space-y-2 text-sm">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <dd className="font-mono font-medium">{result.data.certificateNumber}</dd>
                </div>
                <div className="flex items-center justify-between gap-3">
                  <dt className="text-muted">Issued</dt>
                  <dd className="font-medium">{isoDate(result.data.issuedAt)}</dd>
                </div>
              </dl>
              {!result.data.valid ? (
                <EmptyBlock
