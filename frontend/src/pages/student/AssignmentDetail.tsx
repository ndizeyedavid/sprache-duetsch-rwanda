...
export function AssignmentDetail() {
  ...
  const [fullAssessment, setFullAssessment] = useState<MyAssessmentDetail | null>(null);
  const [attempt, setAttempt] = useState<{ id: string } | null>(null);
  const [responses, setResponses] = useState<Record<string, unknown>>({});
  const [startingQuiz, setStartingQuiz] = useState(false);
  const [submittingQuiz, setSubmittingQuiz] = useState(false);
  const [quizError, setQuizError] = useState<string | null>(null);
  const [quizResult, setQuizResult] = useState<{ score: number; maxScore: number; percentage: number; passed: boolean } | null>(null);
  const [timeLeft, setTimeLeft] = useState<number | null>(null);

  // Preload full assessment for question count (must be before early return for hook order)
  useEffect(() => {
    const raw = detail.data as { assessment?: { id: string }; source?: string } | undefined;
  ...
  async function handleStartQuiz() {
    ...
    try {
      ...
      setCheatNotice(null);
      setStarted(true);
      const fa = fullAssessment ?? (await getMyAssessment(assessmentId).catch(() => null));
  ...
  async function handleSubmitQuiz() {
    if (!attempt) {
      if (cheatLocked) setCheatNotice("Submission blocked — too many violations.");
      return;
    }
    if (!fullAssessment) return;
    // If locked, submit even if flagged, otherwise check unanswered
    if (!cheatLocked) {
      const unanswered = fullAssessment.questions.filter((q) => responses[q.question.id] === undefined || responses[q.question.id] === "" || (Array.isArray(responses[q.question.id]) && (responses[q.question.id] as unknown[]).length === 0));
    ...
    try {
      ...
      if (res.status === "GRADED") {
        setQuizResult({ score: res.score ?? 0, maxScore: res.maxScore ?? 0, percentage: res.percentage ?? 0, passed: !!res.passed });
      } else {
        setQuizResult({ score: 0, maxScore: 0, percentage: 0, passed: false });
      }
      localStorage.removeItem(`cheat:ASM:${attempt.id}`);
      if (data.assessment) localStorage.removeItem(`cheat:ASM:${data.assessment.id}`);
      try {
        if (document.fullscreenElement) await document.exitFullscreen();
      } catch {
        // ignore
        ...
        <Panel>
          {!started && !data.submission ? (
            ...
            <AntiCheatGuard
              ...
              onViolation={(type) => {
                if (activity) void reportActivityViolation(activity.id, type).catch(() => {});
              }}
              onLock={() => {
                setCheatLocked(true);
                setCheatNotice("Assignment locked — 3 violations. Auto-submitting and flagged for teacher review.");
                if (activity) {
                  const v = answer.trim() ? answer : "[No answer — auto-submitted due to violations]";
        ...
        <Panel>
          <div className="rounded-box border border-line bg-base-200/40 p-4">
            ...
            <ul className="mt-2 list-disc pl-5 text-xs text-muted">
              ...
              <li>
                Duration:{" "}
                {fullAssessment?.durationMinutes ?? data.assessment?.durationMinutes
                  ? `${fullAssessment?.durationMinutes ?? data.assessment?.durationMinutes} min`
                  : "Untimed"}
              </li>
              <li>Attempts: {data.attempts?.length ?? 0} / {fullAssessment?.maxAttempts ?? "—"}</li>
              {fullAssessment?.passMark !== null && fullAssessment?.passMark !== undefined ? (
          ...
          {quizResult ? (
            <div className="mt-4 space-y-4 text-center">
              <div className={`mx-auto flex size-14 items-center justify-center rounded-full ${quizResult.passed ? "bg-brand-soft text-brand" : "bg-coral-soft text-coral"}`}>
                <FiCheck aria-hidden className="text-xl" />
              </div>
              <h3 className="text-lg font-bold">{quizResult.passed ? "Passed!" : "Submitted"}</h3>
              <p className="text-sm">
                Score: {quizResult.score} / {quizResult.maxScore} ({quizResult.percentage}%)
              </p>
              <p className="text-xs text-muted">{quizResult.passed ? "Congratulations!" : "Keep practicing and try again if allowed."}</p>
              <div className="flex justify-center gap-2">
                <Link to="/assignments" className="btn btn-sm rounded-full border-line bg-base-100">
                  Back to assignments
                </Link>
                <Link to="/grades" className="btn btn-sm rounded-full border-0 bg-brand text-white">
                  View grades
                </Link>
              </div>
            </div>
            ...
            <>
              <div className="mt-3 rounded-box border border-warning/20 bg-warning/5 p-3">
                <p className="text-xs font-bold text-warning">Anti-cheat active on start</p>
                <p className="text-xs text-muted">Fullscreen required · clipboard blocked · tab switch = violation · 3 violations lock & auto-submit.</p>
              </div>
              {quizError ? <p role="alert" className="mt-2 rounded-box bg-coral-soft px-3 py-2 text-xs font-medium text-coral">{quizError}</p> : null}
              <button
                type="button"
                disabled={startingQuiz}
                onClick={() => void handleStartQuiz()}
                className="btn mt-3 gap-2 rounded-full border-0 bg-brand px-6 text-white hover:bg-brand/90 disabled:opacity-60"
              >
                {startingQuiz ? <span className="loading loading-spinner loading-xs" /> : null}
                Start quiz — enter fullscreen
              </button>
              {detail.data &&
              (detail.data as { attempts?: unknown[] }).attempts &&
              (detail.data as { attempts: { status: string }[] }).attempts.length > 0 ? (
                <p className="mt-3 text-xs text-muted">You have {data.attempts?.length} previous attempt(s).</p>
              ) : null}
            </>
          ) : (
            <AntiCheatGuard
              enabled={started && !cheatLocked && !quizResult}
              persistKey={attempt ? `cheat:ASM:${attempt.id}` : data.assessment ? `cheat:ASM:${data.assessment.id}` : undefined}
              onViolation={(type) => {
                if (attempt) void reportAttemptViolation(attempt.id, type).catch(() => {});
              }}
              onLock={() => {
                setCheatLocked(true);
                setCheatNotice("Assessment locked — 3 violations. Auto-submitting…");
                void handleSubmitQuiz();
              }}
            >
              <div className="mt-3 space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2 rounded-box bg-base-200 px-3 py-2 text-xs">
                  <span className="font-semibold">
                    {fullAssessment?.title ?? data.assessment?.title} · {fullAssessment?.questions.length ?? 0} questions
                  </span>
                  {timeLeft !== null ? (
                    <span className={`rounded-full px-2.5 py-1 font-bold ${timeLeft < 60 ? "bg-error text-white" : timeLeft < 300 ? "bg-warning text-white" : "bg-base-100"}`}>
                      {Math.floor(timeLeft / 60)}:{String(timeLeft % 60).padStart(2, "0")}
                    </span>
                  ) : null}
                </div>
                {cheatNotice ? (
                  <p role="alert" className="rounded-box bg-error px-3 py-2 text-xs font-bold text-white">
                    {cheatNotice}
                  </p>
                ) : null}
                {quizError ? <p role="alert" className="rounded-box bg-coral-soft px-3 py-2 text-xs font-medium text-coral">{quizError}</p> : null}

                {fullAssessment ? (
                  <div className="space-y-4">
                    {fullAssessment.questions.map((q, idx) => {
                      const opts = q.question.options as unknown;
                      const optList = Array.isArray(opts) ? (opts as string[]) : [];
                      const qType = q.question.type;
                      const val = responses[q.question.id];
                      return (
                        <div key={q.question.id} className="rounded-box border border-line bg-base-100 p-4">
                          <p className="flex gap-2 text-sm font-semibold">
                            <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-brand text-xs font-bold text-white">{idx + 1}</span>
                            {q.question.prompt}
                          </p>
                          {q.question.imageUrl ? <img src={q.question.imageUrl} alt="" className="mt-2 max-h-48 rounded-box" /> : null}
                          {q.question.audioUrl ? <audio controls src={q.question.audioUrl} className="mt-2 w-full" /> : null}
                          <div className="mt-3">
                            {qType === "SINGLE_CHOICE" || qType === "TRUE_FALSE" ? (
                              <div className="space-y-2">
                                {(optList.length ? optList : qType === "TRUE_FALSE" ? ["True", "False"] : []).map((opt, oIdx) => (
                                  <label key={oIdx} className={`flex cursor-pointer items-center gap-2 rounded-box border p-3 text-sm ${val === opt || val === String(oIdx) ? "border-brand bg-brand-soft" : "border-line bg-base-100 hover:border-brand/20"}`}>
                                    <input
                                      type="radio"
                                      name={q.question.id}
                                      checked={val === opt || val === String(oIdx) || val === oIdx}
                                      onChange={() => setResponses((p) => ({ ...p, [q.question.id]: opt }))}
                                      className="radio radio-sm"
                                      disabled={cheatLocked}
                                    />
                                    {opt}
                              ...
                              <div className="space-y-2">
                                {optList.map((opt, oIdx) => {
                                  ...
                                  return (
                                    <label key={oIdx} className={`flex cursor-pointer items-center gap-2 rounded-box border p-3 text-sm ${checked ? "border-brand bg-brand-soft" : "border-line bg-base-100"}`}>
                                      <input
                                        type="checkbox"
                                        checked={checked}
                                        onChange={(e) => {
                                          const next = e.target.checked ? [...arr, opt] : arr.filter((x) => x !== opt);
                                          setResponses((p) => ({ ...p, [q.question.id]: next }));
                                        }}
                              ...
                              <input
                                value={(val as string) ?? ""}
                                onChange={(e) => setResponses((p) => ({ ...p, [q.question.id]: e.target.value }))}
                                placeholder="Your answer…"
                                disabled={cheatLocked}
                          ...
                          <p className="mt-2 text-xs text-muted">{q.question.points} pts</p>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {submittingQuiz ? <span className="loading loading-spinner loading-xs" /> : null}
                    Submit quiz
                  </button>
                </div>
              </div>
