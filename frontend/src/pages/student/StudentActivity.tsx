...
import { getMyCourses, getStudentLesson, submitActivity } from '../../lib/services';
...
export function StudentActivity() {
  ...
  function handleRedo() {
    ...
    setFeedback(null);
    setSubmitError(null);
    // keep previous answer for convenience, but allow editing; clear choice states?
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Panel>
        <span className="inline-flex items-center gap-2 rounded-full bg-brand-soft px-2.5 py-1 text-xs font-semibold text-[#B30A00]"><FiClock aria-hidden />Practice activity</span>
      ...
      <Panel>
        {!submitted ? (
          <div className="space-y-4">
            ...
            <button type="button" onClick={() => void handleSubmit()} disabled={submitting || (type === 'MCQ' ? mcqChoice === null : type === 'TRUE_FALSE' ? tfChoice === null : !answer.trim())} className="btn rounded-full border-0 bg-brand px-6 text-white hover:bg-brand/90 disabled:opacity-50">{submitting ? <span className="loading loading-spinner loading-xs" /> : null}Submit</button>
          </div>
        ) : (
