import { FiArrowRight, FiBookOpen, FiCalendar, FiTrendingUp } from 'react-icons/fi';
...
import { getStudentDashboard, getUpcomingSessions, isoDate, isoTime, money } from '../../lib/services';
      ...
      <div className="space-y-5 xl:col-span-8">
        <section className="card-shadow relative overflow-hidden rounded-box bg-brand text-white">
          <div className="relative z-10 max-w-md p-6 sm:p-8">
            <p className="text-[11px] font-medium uppercase tracking-wide text-white/70">
              {data.currentLevel ? `${data.currentLevel.code} · ${data.currentLevel.title}` : 'No active level'}
            </p>
            <h2 className="mt-1 text-xl font-semibold leading-snug sm:text-2xl">
              Weiter geht&apos;s — {data.progress.lessonsCompleted} of {data.progress.lessonsTotal} lessons done
            </h2>
            <p className="mt-2 text-xs leading-relaxed text-white/80 sm:text-sm">
              {data.campus ? `${data.campus.name} · ` : ''}
              {data.classGroup ? `${data.classGroup.name}` : 'No class assigned yet.'}
              {data.finance.balance ? ` · Balance ${rwf(money(data.finance.balance))}` : ''}
            </p>
          </div>
          <img
            src={hero}
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          <Panel className="lg:col-span-2">
            <h2 className="mb-4 text-base font-semibold sm:text-lg">Attendance breakdown</h2>
            <GroupedBar
              data={attendanceBars}
              xKey="label"
              barSize={22}
          ...
          <Panel className="flex flex-col items-center justify-center text-center">
            <RadialStat value={data.progress.completionPercentage} size={168} className="mx-auto">
              <span className="text-2xl font-semibold text-ink">{data.progress.completionPercentage}%</span>
        ...
        <Panel>
          <h2 className="mb-4 text-base font-semibold sm:text-lg">Learning status</h2>
          <ProgressRow
          ...
          {data.nextExam ? (
            <p className="mt-3 text-xs text-muted">
              Next exam: <span className="font-semibold text-ink">{data.nextExam.title}</span> ({data.nextExam.type})
            </p>
          ) : (
            <p className="mt-3 text-xs text-muted">No upcoming exam scheduled.</p>
          )}
        </Panel>
      </div>
      ...
      <div className="space-y-5 xl:col-span-4">
        <Panel>
          ...
          {upcoming.loading ? (
            ...
            <EmptyBlock title="No upcoming classes" hint="Your teacher has not scheduled the next live class yet." />
          ) : (
