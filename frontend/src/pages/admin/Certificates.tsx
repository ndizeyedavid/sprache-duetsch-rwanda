    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <Panel>
        ...
        <form onSubmit={handleCheck} className="space-y-3">
          ...
          >
            {checking ? <span className="loading loading-spinner loading-sm" /> : 'Check eligibility'}
          </button>
        </form>
        ...
        {eligibility ? (
          <div className="mt-4 rounded-field bg-base-200 p-4 text-xs">
            ...
            {eligibility.reasons.length > 0 ? (
              ...
              <p className="mt-2 font-semibold text-brand">Eligible — ready to issue.</p>
            )}
            <button
              type="button"
            ...
            >
              {working ? <span className="loading loading-spinner loading-sm" /> : 'Issue certificate'}
            </button>
          </div>
        ) : null}
      ...
      <Panel>
        ...
        {issued.loading ? (
          ...
          <EmptyBlock title="No certificates yet" hint="Issued certificates appear here with revoke controls." />
        ) : (
          <ul className="space-y-3">
            {list.map((certificate) => (
              <li key={certificate.id} className="rounded-field bg-base-200 px-3 py-2">
                ...
                {certificate.status === 'ISSUED' ? (
                  <span className="mt-2 flex gap-2">
                    <input
                      ...
                      onChange={(event) =>
                        setRevokeReason((current) => ({ ...current, [certificate.id]: event.target.value }))
                      }
                      placeholder="Revoke reason"
                      aria-label={`Revoke reason for ${certificate.certificateNumber}`}
                      className="input input-sm grow rounded-field border-line bg-base-100"
                    />
                    <button
                      type="button"
                      disabled={working}
